...
public:
	...
	explicit TQuaternion(T s, TVector<T, 3> v) : m_vector(MoveTemp(v)), m_scalar(s) {}
	...
	Iterator end() { return &m_scalar + 1; }
	ConstIterator begin() const { return m_vector.begin(); }
	ConstIterator end() const { return &m_scalar + 1; }
	T& Data(int index) { return *(begin() + index); }
	T Data(int index) const { return *(begin() + index); }
	T GetScalar() const { return m_scalar; }
	void SetScalar(T s) { m_scalar = s; }
	const TVector<T, 3>& GetVector() const { return m_vector; }
	void SetVector(TVector<T, 3> v) { m_vector = MoveTemp(v); }
	T x() const { return m_vector.x(); }
	T y() const { return m_vector.y(); }
	T z() const { return m_vector.z(); }
	T w() const { return m_scalar; }
	T& x() { return m_vector.x(); }
	T& y() { return m_vector.y(); }
	T& z() { return m_vector.z(); }
	T& w() { return m_scalar; }

	// Validations
	bool IsNaN() const { return std::isnan(m_scalar) || std::isnan(m_vector.x()) || std::isnan(m_vector.y()) || std::isnan(m_vector.z()); }
	...
	// Calculations
	TQuaternion<T> Inverse() const { return TQuaternion<T>(m_scalar, -m_vector); }
	T Dot(const TQuaternion& rhs) const { return m_scalar * rhs.m_scalar + m_vector.x() * rhs.m_vector.x() + m_vector.y() * rhs.m_vector.y() + m_vector.z() * rhs.m_vector.z(); }
	T LengthSqure() const { return Dot(*this);  }
	T Length() const { return std::sqrt(LengthSqure());  }
	...
	// Operators
	TQuaternion<T> operator+(const TQuaternion<T>& rhs) const { return TQuaternion<T>(m_scalar + rhs.m_scalar, m_vector + rhs.m_vector); }
	TQuaternion<T>& operator+=(const TQuaternion<T>& rhs) { m_scalar += rhs.m_scalar; m_vector += rhs.m_vector; return *this; }
	
	TQuaternion<T> operator-() const { return TQuaternion<T>(-m_scalar, -m_vector.x(), -m_vector.y(), -m_vector.z()); }
	TQuaternion<T> operator-(const TQuaternion<T>& rhs) const { return TQuaternion<T>(m_scalar - rhs.m_scalar, m_vector - rhs.m_vector); }
	TQuaternion<T>& operator-=(const TQuaternion<T>& rhs) { m_scalar -= rhs.m_scalar; m_vector -= rhs.m_vector; return *this; }

	friend TQuaternion<T> operator*(T lhs, const TQuaternion<T>& rhs) { return rhs * lhs; }
	TQuaternion<T> operator*(T scalar) const { return TQuaternion<T>(m_scalar * scalar, m_vector.x() * scalar, m_vector.y() * scalar, m_vector.z() * scalar); }
	...
	bool operator==(const TQuaternion& rhs) const { return m_scalar == rhs.m_scalar && m_vector == rhs.m_vector; }
	bool operator!=(const TQuaternion& rhs) const { return !(*this == rhs); }
